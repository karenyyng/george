...
cdef extern from "kernels.h" namespace "george::kernels":
    ...
    cdef cppclass KappaKappaExpSquaredKernel[M](DerivativeExpSquaredKernel):
        KappaKappaExpSquaredKernel(const unsigned int ndim, M* metric)

    cdef cppclass KappaGamma1ExpSquaredKernel[M](DerivativeExpSquaredKernel):
        KappaGamma1ExpSquaredKernel(const unsigned int ndim, M* metric)

    cdef cppclass KappaGamma2ExpSquaredKernel[M](DerivativeExpSquaredKernel):
        KappaGamma2ExpSquaredKernel(const unsigned int ndim, M* metric)

    cdef cppclass Gamma1Gamma1ExpSquaredKernel[M](DerivativeExpSquaredKernel):
        Gamma1Gamma1ExpSquaredKernel(const unsigned int ndim, M* metric)

    cdef cppclass Gamma1Gamma2ExpSquaredKernel[M](DerivativeExpSquaredKernel):
        Gamma1Gamma2ExpSquaredKernel(const unsigned int ndim, M* metric)

    cdef cppclass Gamma2Gamma2ExpSquaredKernel[M](DerivativeExpSquaredKernel):
...
cdef inline Kernel* parse_kernel(kernel_spec) except *:
    ...
    if kernel_spec.kernel_type >= 10:
        if len(kernel_spec.pars) != ndim:
            print "trying to reset float value to vector"
            kernel_spec.pars = kernel_spec.pars * np.ones(ndim)
    ...
    if kernel_spec.kernel_type == -2:
        ...
        if kernel_spec.axis_aligned:
            kernel = new KappaKappaExpSquaredKernel[AxisAlignedMetric](ndim,
                new AxisAlignedMetric(ndim))
        else:
            raise NotImplementedError("Metric needs to be a list / array of len" +
                                      "({0}) to match ndim = {0}".format(ndim))
